...
## 11. 実機で「かぶってない」指摘を受けての修正（配置の基準を変えた）

実機（1643x792 相当・aspect 2.07）のスクリーンショットで、円柱が文字の右外に
出ていて重なっていないとの指摘。

### 原因：配置の基準が「箱の中心」だった

`right = visibleWidth * 比率` で置いていた。**文字は左端（ステージのパディング）に
固定なのに、箱の中心は画面幅の半分**なので、横長になるほど円柱が文字から離れる。
検証を 1440x900（aspect 1.6）だけでやっていて気づかなかった。

### 直し方：文字の矩形を実測して、その右寄りを狙う

`useSkillIntroStage` に `focusRef`（見出しの span）を渡し、`onResize` で

1. 文字の矩形を `Range.getBoundingClientRect()` で測る
2. その `x` の 86% 地点・`y` 中央よりやや上を狙う点にする
3. 箱の中心からの画面上のずれ（px）を出す
4. `pxPerWorld = canvas.clientHeight / visibleHeight` でワールドに直す
5. 35deg の逆回転を掛けて `group.position` に入れる

文字が測れない場合は箱の中心のやや上に置く（従来の挙動）。

### 修正後の実測（手前 canvas の写真と文字の重なり）

片方の canvas だけを表示したスクリーンショットと、両方隠したものの
**画素差の bbox** を取って比較した。

| 画面 | aspect | 文字 | 手前の写真 | 重なり |
|---|---|---|---|---|
| 1440x900 | 1.60 | x56-830 y211-375 | x460-983 y65-452 | **370x164px** |
| 1643x792（実機相当） | 2.07 | x56-830 y157-321 | x493-947 y65-369 | **337x164px** |
| 1920x1080 | 1.78 | x56-830 y301-465 | x398-1034 y65-588 | **432x164px** |
| 2560x1080 | 2.37 | x56-830 y301-465 | x397-1021 y69-583 | **433x164px** |
| 390x780 | 0.50 | x20-256 y91-141 | x191-333 y65-188 | 65x50px（文字の全高） |

y 方向は全ケースで文字の高さぶん重なっている。

### 開発中に見えている絵が古いことがある（HMR）

`useSkillIntroStage` の `useEffect` の依存は `[backRef, frontRef, focusRef]` で
すべて安定した ref なので、**モジュール定数（`CYL_LAYOUT_*` など）を書き換えても
effect は再実行されない。** `layoutCylinder` は `onResize` からしか呼ばれないため、
Fast Refresh のあとも古い位置のままになる。数値を触ったら**フルリロード**すること。
